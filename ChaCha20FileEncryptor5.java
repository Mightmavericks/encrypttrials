...
public class ChaCha20FileEncryptor5 {
    ...
    private static void decryptFileAESCTRToFinal(File inputFile, File outputFile, SecretKey aesKey, SecretKey chachaKey, byte[] nonce, byte[] iv) throws Exception {
        ...
        // Decrypt AES-CTR layer
        byte[] decryptedAESBytes = aesCipher.doFinal(encryptedBytes);
        ...
        byte[] decryptedChaChaBytes = chachaCipher.doFinal(encryptedChaChaBytes);

        // Extract the original content
        byte[] originalBytes = new byte[originalLength];
        System.arraycopy(decryptedChaChaBytes, 0, originalBytes, 0, originalLength);
